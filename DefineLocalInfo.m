function [Surf] = DefineLocalInfo(Surf)
% %
...
%     Surf.interps{j} = cell2mat(possiblePts)';
    ...
    for k = 1:Surf.nVFs
        ...
        %Compute Weights
        W{k,j} = sparse(Surf.nPts,length(Surf.Template));
...
%         size(C)
        for PTidx = 1:length(Surf.LCoeff{k,j,1})
            ...
            for TRGidx = 1:length(TR.ConnectivityList)
                ...
                if min(Coords) >= 0
                    W{k,j}(Surf.interps{j}(PTidx),TR(TRGidx,:)) = Coords;%THIRD ARGUEMENT 
                    break
                end   
            end
        end
...
Wfull = cell2mat(W');
for k = 1:Surf.nVFs
    first = (k-1)*length(Surf.Template)+1;
    last  = (k)*length(Surf.Template);
    Surf.Wmat{k} = Wfull(:,first:last);
end