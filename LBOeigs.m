function [eigFun, eigVal, MassV,MassM, StiffM] = LBOeigs(surf,numEigs)
...
%fullMass = diag(MassV);
MassM = sparse(1:length(pts),1:length(pts),MassV);

%Find and sort eigs
[eigFun, eigVal] = eigs(StiffM+MassM,MassM,numEigs,'sm');
eigVal = diag(eigVal);
