function [MassM,StiffM] = MassStiffMtx(varargin)

%%%%%%%%  [MassM,StiffM,cputime] = MassStiffMtx(S)
%%%%%%%   S.pt :  mesh coordinates
%%%%%%%   S.trg:  mesh connectivity
%%%%%%%  This funciton solves the Laplace-Belatrmai Eigenvalue problem
...
% Mass = sparse(i,j,ss.*(sarea(dii,dji).*Wqi + sarea(dij,djj).*Wqj)./12,N,N);
% Mass = 0.5*(Mass + Mass');
% Mass = Mass + diag(sum(Mass,2));
...
% TrgArea = sparse(i,j,ss.*(sarea(dii,dji) + sarea(dij,djj)),N,N);
% aroundpttrgarea = sum(TrgArea,2)/2;



% % tic;
% % [tempV,tempD] = eigs(StiffM,MassM,num_eigen,'sm');
% % cputime = toc;
% % tempD = diag(tempD);
% % 
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
